...
class CaptioningRNN(object):
  ...
  def loss(self, features, captions):
    ...
    ############################################################################
    out_affine, cache_affine = affine_forward(features, W_proj, b_proj)# (N, H)
    word_vector, cache_embed = word_embedding_forward(captions_in, W_embed) #(N, T, H)
    
    if self.cell_type=='rnn':
      word_ranks, caches_rnn = rnn_forward(word_vector, out_affine, Wx, Wh, b)
    elif self.cell_type == 'lstm':
      word_ranks, caches_lstm = lstm_forward(word_vector,out_affine,Wx, Wh, b)
    ...
    if self.cell_type == "rnn":
      ...
      dword_vector, dout_affine, grads['Wx'], grads['Wh'], grads['b'] = lstm_backward(dword_ranks, caches_lstm)
  ...
  def sample(self, features, max_length=30):
    ...
    if self.cell_type == "rnn":
      ...
      prev_c = np.zeros((n,H))                  
      next_h,next_c, cache0 = lstm_step_forward(word_embedding[:,0,:], out_affine,prev_c, Wx, Wh, b ) #N, 0, H
    h[:,1,:] = next_h
    ...
    for time_step in range(0,len(captions[0,:])-2):
      if self.cell_type == "rnn":
        ...
        next_h,next_c, cache = lstm_step_forward(word_embedding[:,time_step+2,:],next_h,next_c, Wx, Wh, b )
